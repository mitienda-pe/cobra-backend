...
class PaymentController extends ResourceController
{
    ...
    public function generateInstalmentQR($instalmentId = null)
    {
        ...
        // Modificamos la estrategia: en lugar de intentar usar el método privado createLigoOrder,
        // vamos a simular una factura y usar el método público generateQR
        
        // Creamos una factura temporal con los datos de la cuota
        $tempInvoice = [
            'id' => 'instalment-' . $instalment['id'],
            'amount' => $instalment['amount'],
            'currency' => $invoice['currency'] ?? 'PEN',
            'invoice_number' => $invoice['invoice_number'] . '-C' . $instalment['instalment_number'],
            'organization_id' => $invoice['organization_id'],
            'client_id' => $invoice['client_id'],
            'portfolio_id' => $invoice['portfolio_id']
        ];
        
        // Usamos el método público generateQR con los datos de la cuota
        $response = $ligoController->generateQR($instalment['id']);
        
        // Si la respuesta es un objeto Response, extraemos su contenido
        if (method_exists($response, 'getBody')) {
            $responseBody = json_decode($response->getBody());
            
            if (!$responseBody || !isset($responseBody->success) || !$responseBody->success) {
                return $this->fail('Error generating QR for instalment', 400);
            }
            
            // Devolvemos la respuesta con información adicional de la cuota
            return $this->respond([
                'success' => true,
                'qr_data' => $responseBody->qr_data ?? null,
                'qr_image_url' => $responseBody->qr_image_url ?? null,
                'order_id' => $responseBody->order_id ?? null,
